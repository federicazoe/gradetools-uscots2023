...
There are `r nrow(babies_filtered)` observations and `r ncol(babies_filtered)` variables in the data frame.
...
No that's for continuous variables. I'd do a box plot colored by smoking habit.
...
*I couldn't get the plot to work like in the picture.*

```{r}
ggplot(babies_filtered, aes(x = gestation, y = wt)) +
  geom_point() +
  facet_wrap(smoke ~ ed, nrow = 4)
```
